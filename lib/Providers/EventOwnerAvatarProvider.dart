...
class EventOwnerAvatarProvider extends ChangeNotifier {
  ...
  Future<void> uploadAvatar(String yourId, File? avatar, String? token) async {
    setLoading(true);
    String url = "http://192.168.0.136:8080/api/avatarowner/upload/$yourId";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setUploaded(false);
        setError(true);
        setErrorText(jsonDecode(Utf8Decoder(allowMalformed: true).convert(response.bodyBytes))["message"]);
  ...
  Future<void> changeAvatar(String yourId, File? avatar, String? token) async {
    String url = "http://192.168.0.136:8080/api/avatarowner/update/$yourId";
    // Map<String, String> imageMap = {"file": basename(image!.path)};
    // var body = jsonEncode(imageMap);
    try {
      // http.Response response = await http.post(Uri.parse(url),
